...
%                                                                         %
%                              INSTRUCTIONS                               %
%   1) Run datalogConvertV4.m                                             %
%   2) Select the datalog files to convert (usually .asc extension)       %
%       NOTE: This code will work with any of the 3 datalog files         %
%   3) Choose a filename and path to save the file. The file can be saved %
%       as the following filetypes:                                       %
%           *.mat - MATLAB workspace containing data vectors              %
%           *.asc - ASCII text file                                       %
%           *.txt - standard text format (space-delimited)                %
%           *.dat - data format (space-delimited)                         %
%           *.xlsx - Excel spreadsheet (2007 and later)                   %
%           *.xlsm - macro-enabled Excel spreadsheet (2007 and later)     %
%           *.xls - Excel spreadsheet (97-2003)                           %
%           *.csv - comma separated value file/spreadsheet                %
%       NOTE: Saving as *.mat is fastest, followed by *.xls, *.xlsx, and  %
%             *.xlsm                                                      %
%             *.asc, *.dat, and *.csv are slowest                         %
%   4) Verify the newly created file has the proper headings. The         %
%       headings are in the first line/row.                               %
%                                                                         %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clc;clear;close all

warning('off','all')    %deactivate command window warnings

mods.Interpreter = 'tex';   %error dialog box options
mods.WindowStyle = 'modal';

if exist('datalogConvertV4lastpath.mat','file') == 2
    load('datalogConvertV4lastpath.mat')    %recall last filepaths used
else
    lastloadpath = cd;  %default load filepath to current directory
    lastsavepath = cd;  %default save filepath to current directory
end

%Prompt to load datalog file(s)
[loadname,loadpath] = uigetfile({'*.asc';'*.txt';'*.dat';'*.*'},...
    'Select One or More Files',lastloadpath,'MultiSelect','on');
if ~iscell(loadname) && all(loadname == 0)  %if load operation is cancelled
    f = errordlg('\fontsize{12}No file selected.',...
        'Select One or More Files',mods);   %creates error dialog box
    error('Error loading data. No file selected.')  %ends script early
end

%If single filename, convert to 1x1 cell for compatibility with for loops
if ~iscell(loadname)
    temp = loadname;
    clearvars loadname
    loadname{1} = temp;
end
numfiles = length(loadname);    %number of datalog files to load

%Naming new file(s)
savename{numfiles} = '';        %preallocation
saveind = zeros(1,numfiles);
for i = 1:numfiles
    %Get filename without file extension
    name = loadname{i}(1:strfind(loadname{i},'.')-1);
    
    %Prompt to name each saved file
    [savename{i},savepath,saveind(i)] = uiputfile({'*.mat';'*.asc';...
        '*.txt';'*.dat';'*.xlsx';'*.xlsm';'*.xls';'*.csv'},'Save File',...
        fullfile(lastsavepath,name));
    if savename{i} == 0     %if save operation is cancelled
        f = errordlg('\fontsize{12}Save operation aborted.','Save File',...
            mods);
        error('Error saving data. Save operation aborted.') %ends script
    end
end

%Loading, formatting, and saving each file
ss = zeros(1,numfiles);    %preallocation
for i = 1:numfiles
    ss(i) = loadnsave(loadpath,loadname{i},savepath,savename{i},...
        saveind(i),0);  %returns 1 if save successful
end
...
msg = [{'Save successful.';
    ...
    savename(ss)'];
f = msgbox(msg,'Save File','help');

%Save last used load/save directories
lastloadpath = loadpath;
lastsavepath = savepath;
save('datalogConvertV4lastpath','lastloadpath','lastsavepath')
