...
%% Setup
%Pilot Selection
pilots = 4:9;       %only Cessna_152_v19 pilots

%"Ideal" performance parameters: [min max]
cr = [500 700];     %climb rate, clean [ft/min]
...
Vapp = [70 80];     %approach speed [KIAS]
Vappf = [55 65];    %approach speed w/ flaps [KIAS]
Vclimb = [65 70];   %climb speed [KIAS]
Vs = 48;            %stall speed, clean [kn]
Vs0 = 43;           %stall speed, flaps [kn]
Vfe = 85;           %maximum flaps extended speed [kn]
Va = 104;           %maximum maneuvering speed, max weight [kn]
Vy = 67;            %best ROC speed, sea level [kn]
Vto = 60;           %takeoff speed [kn]
Vtosf = 54;         %short field takeoff speed [kn]
dtosf = 725;        %short field takeoff distance [ft]
V3 = 100;           %speed for cond 3 S-Turn [kn]
V4 = 100;           %speed for cond 4 360 turn [kn]
v8 = 70;            %speed for cond 8 S-Turn w/ flaps [kn]
whead4 = 130;       %wind heading for cond 4 [degrees]
wspeed4 = 5;        %wind speed for cond 4 [kn]
whead5 = 130;       %wind heading for cond 5 [degrees]
wspeed5 = 5;        %wind speed for cond 5 [degrees]
...
plot(ax4,ax4.XLim,Vto*ones(1,2),'k--','LineWidth',2)
...
for k = 1:nt
    for i = pilots
        if isempty(s(i,cond,k).time)    %check for missing data
            ...
            if t(end) > 150 %shorten data if longer than 2.5 minutes
                ind2 = find(t>=150,1);
...
whead = 130;    %heading that wind comes FROM [degrees]
wspeed = 5;     %wind speed [kn]
x0 = 0.7;       %arrow start x-coord [0-1]
y0 = 0.7;       %arrow start y-coord [0-1]
r = 0.1;        %arrow length

whead = whead+180;  %reverse direction for correct arrow
x = x0 + r*[0,sind(whead)];
y = y0 + r*[0,cosd(whead)];
str = sprintf('%i kn from %i\\circ',wspeed,whead-180);
...
figure
hold on
ax7 = gca;  %get axes for altitude plot (takeoff)
...
%             indto = find(hdot>30,1);        %find takeoff from runway
            indto = find(alt<4,1,'last');   %find takeoff from runway
            indf = find(df==1,1,'last');    %find when flaps are retracted
...
%                 norm([N(indto1),E(indto1)])-norm([N(indto),E(indto)]))
            ...
            plot(ax7,tTO,altTO,'Color',cp(i-pilots(1)+1,:)*ct(k))
            plot(ax7,tTO(indto),altTO(indto),'*','Color',cp(i-pilots(1)+1,:)*ct(k))
        end
    end
end
...
grid(ax6,'on')
xlabel(ax6,'Time (seconds)')
ylabel(ax6,'Vertical Speed (ft/min)')
if showlgd
    legend(ax6,lgdcell,'NumColumns',nt)
end
title(ax6,titstr)
